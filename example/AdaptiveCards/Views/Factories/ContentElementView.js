...
export class ContentElementView extends React.Component {
    ...
    renderSeparator() {
        if (this.props.element.separator && this.props.index !== 0) {
            return (React.createElement(SeparateLine, { key: 'separator' + this.props.index, isHorizontal: DecStyleManager.getInstance().isHorizontalCardElement(this.props.element.type), margin: DecStyleManager.getInstance().getCardElementMargin(this.props.element.spacing), color: DecStyleManager.getInstance().getStyle().element.separateLineColor }));
        }
    }
}
